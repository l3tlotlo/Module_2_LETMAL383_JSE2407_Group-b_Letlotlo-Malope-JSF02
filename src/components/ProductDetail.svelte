<script>
    ...
    function addToCart() {
      const event = new CustomEvent('add-to-cart', { detail: selectedProduct });
      dispatchEvent(event);
    }
    ...
    <div class="bg-white rounded-lg shadow-md p-4 max-w-3xl mx-auto">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <button on:click={closeDetail} class="bg-gray-500 text-white py-2 px-4 rounded mt-2">Close</button>
        </div>
      </div>
    </div>
  </div>
